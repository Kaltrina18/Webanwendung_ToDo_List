...
function validatePasswordRules(pw) {
  if (!pw || pw.length < 12) return 'Passwort muss mindestens 12 Zeichen haben.';
  if (!/[!@#$%^&*()_+\-=[\]{};':"\\|,.<>/?~]/.test(pw)) return 'Passwort muss mindestens ein Sonderzeichen enthalten.';
  return null;
}
...
app.post('/api/request-code', (req, res) => {
  ...
  if (!identifier) return res.status(400).json({ error: 'Identifier fehlt' });
  const code = crypto.randomInt(100000, 999999).toString();
  db.get('SELECT id FROM users WHERE identifier = ?', [identifier], (err, row) => {
    if (err) return res.status(500).json({ error: err.message });
    if (row) {
      db.run('UPDATE users SET verification_code = ?, verified = 0 WHERE id = ?', [code, row.id], e=>{
        if(e) return res.status(500).json({ error:e.message });
        sendVerificationCode(identifier, code).catch(()=>{});
        return res.json({ status:'ok', debugCode: code });
      });
    } else {
      db.run('INSERT INTO users (identifier, verification_code) VALUES (?, ?)', [identifier, code], e=>{
        if(e) return res.status(500).json({ error:e.message });
        sendVerificationCode(identifier, code).catch(()=>{});
        return res.json({ status:'ok', debugCode: code });
      });
...
app.post('/api/verify-and-set', (req,res)=>{
  const { identifier, code, password, purpose } = req.body;
  if(!identifier || !code || !password) return res.status(400).json({ error:'Fehlende Daten' });
  const pwErr = validatePasswordRules(password);
  if(pwErr) return res.status(400).json({ error: pwErr });

  db.get('SELECT id, verification_code FROM users WHERE identifier = ?', [identifier], (err,row)=>{
    if(err) return res.status(500).json({ error: err.message });
    if(!row) return res.status(400).json({ error:'Benutzer nicht gefunden' });
    if(row.verification_code !== code.toString()) return res.status(400).json({ error:'Code falsch' });

    const hash = bcrypt.hashSync(password, 10);
    db.run('UPDATE users SET verified=1, verification_code=NULL, password_hash=? WHERE id=?', [hash,row.id], e=>{
      if(e) return res.status(500).json({ error:e.message });
...
app.post('/api/login', (req,res)=>{
  ...
  if(!identifier || !password) return res.status(400).json({ error:'Identifier + Passwort benötigt' });
  db.get('SELECT id, verified, password_hash FROM users WHERE identifier=?', [identifier], (err,row)=>{
    if(err) return res.status(500).json({ error: err.message });
    if(!row) return res.status(400).json({ error:'Benutzer nicht gefunden' });
    if(!row.verified) return res.status(400).json({ error:'Nicht verifiziert' });
    if(!row.password_hash) return res.status(400).json({ error:'Kein Passwort gesetzt' });
    if(!bcrypt.compareSync(password,row.password_hash)) return res.status(400).json({ error:'Falsches Passwort' });
...
app.post('/api/todos', requireAuth, (req,res)=>{
  ...
  const today = new Date().toISOString().split('T')[0];
  db.run('INSERT INTO todos (text, done, owner_id, createdDate) VALUES (?,?,?,?)', [text,0,req.session.userId,today], function(e){
    if(e) return res.status(500).json({ error: e.message });
...
app.post('/api/todos/:id/toggle', requireAuth, (req,res)=>{
  ...
  db.get('SELECT done, owner_id FROM todos WHERE id=?',[id],(err,row)=>{
    ...
    db.run('UPDATE todos SET done=?, doneDate=? WHERE id=?',[newDone, doneDate, id], e=>{
      if(e) return res.status(500).json({ error:e.message });
      res.json({ status:'ok', done:newDone });
    });
  });
});
